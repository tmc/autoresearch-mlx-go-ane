...
func extractIOSurface(surface any) (*IOSurfaceFloat32, error) {
	if surf, ok := surface.(*IOSurfaceFloat32); ok {
		return surf, nil
	}
	...
	if rs, ok := surface.(rawSurfacer); ok {
		if surf, ok := rs.RawSurface().(*IOSurfaceFloat32); ok {
			return surf, nil
		}
	}
...
func compactOutput(surf *IOSurfaceFloat32) error {
...
func (decodePlaneRuntime) NewQwen35Stage(dim, hidden int, cacheDir string, w1, w3, w2 []float32) (any, any, error) {
	...
	bridge, err := NewMLXSurfaceSyncBridge(plan.WaitEvent(), plan.SignalEvent())
	if err != nil {
		stage.Close()
...
func (decodePlaneRuntime) NewQwen35DirectBlock(prog any, cfg offload.DirectBlockConfig) (any, any, error) {
	...
	planCfg := DefaultMultiSurfaceEvalPlanConfig()
	...
	if selectedLayers > 1 {
		draft, _, err = NewANEDraftModelFromModelIRProgramLayerStackWithPlanConfig(
			modelProg,
			reifyOpts,
			cfg.HiddenDim,
			cfg.VocabSize,
		...
		draft, _, err = NewANEDraftModelFromModelIRProgramWithPlanConfig(
			modelProg,
			reifyOpts,
			cfg.HiddenDim,
			cfg.VocabSize,
	...
	bridge, err := NewMLXSurfaceSyncBridge(draft.WaitEvent(), draft.SignalEvent())
	if err != nil {
		draft.Close()
