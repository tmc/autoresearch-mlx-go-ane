...
func TestFFNStageAccuracy(t *testing.T) {
	...
	if err := GenerateFFNEspressoDir(stageDir, dim, hidden, gate, up, down); err != nil {
	...
	model2, err := CompileAndLoadEspresso(client, stageDir, "", 0)
	if err != nil {
	...
	plan, err := NewSurfaceEvalPlanWithClientModel(model2, inputSurf, outputSurf, SurfaceEvalPlanConfig{})
	if err != nil {
